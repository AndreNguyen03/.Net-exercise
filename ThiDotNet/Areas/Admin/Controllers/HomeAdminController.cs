...
namespace WebsiteBanHang.Areas.Admin.Controllers
{
    ...
    public class HomeAdminController : Controller
    {
        ...
        [Route("themmoinguoidung")]
        [HttpGet]
        public IActionResult ThemMoiNguoidung()
        {
            ViewBag.MaChatLieu = new SelectList(db.TChatLieus.ToList(), "MaChatLieu", "ChatLieu");
            ViewBag.MaHangSx = new SelectList(db.THangSxes.ToList(), "MaHangSx", "HangSx");
            ViewBag.MaNuocSx = new SelectList(db.TQuocGia.ToList(), "MaNuoc", "TenNuoc");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult ThemMoiNguoidung(TUser user)
        {
            if (ModelState.IsValid)
            {
                db.TUsers.Add(user);
        ...
        [Route("suasanpham")]
        [HttpGet]
        public IActionResult SuaNguoiDung(string maSp)
        {
            ViewBag.MaChatLieu = new SelectList(db.TChatLieus.ToList(), "MaChatLieu", "ChatLieu");
            ViewBag.MaHangSx = new SelectList(db.THangSxes.ToList(), "MaHangSx", "HangSx");
            ViewBag.MaNuocSx = new SelectList(db.TQuocGia.ToList(), "MaNuoc", "TenNuoc");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult SuaNguoiDung(TDanhMucSp sanPham)
        {
            if (ModelState.IsValid)
            {
                db.Update(sanPham).State = EntityState.Modified;
        ...
        [Route("XoaSanPham")]
        [HttpGet]
        public IActionResult XoaNguoiDung(string maSp)
        {
            TempData["Message"] = "";
            var chiTietSanPham = db.TChiTietSanPhams.Where(x => x.MaSp == maSp).ToList();
            if (chiTietSanPham.Count() > 0)
